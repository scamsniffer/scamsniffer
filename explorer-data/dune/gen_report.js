const { getReportByLinkAddress } = require('./index');
const fs = require('fs');
...
async function getNewDomains(lastId = 39) {
  const req = await fetch("https://api.scamsniffer.io/domainSummary?sort=-id&needReport=1");
...
async function genReport(lastId = 39) {
  ...
  const allList = Array.from(allAttackers.reduce((all, item) => {
  ...
  const report = await getReportByLinkAddress(allList);
