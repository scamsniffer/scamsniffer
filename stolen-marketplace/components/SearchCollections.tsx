...
const SearchCollections: FC<Props> = ({ communityId, initialResults }) => {
  ...
  const debouncedSearch = useCallback(
    debounce(async (value) => {
      ...
      try {
        const href = `${RESERVOIR_API_BASE}/v1/all.json`
        ...
        const match = value
          ? data.filter((_) => {
              return (
                ...
                _.contract_address.toLowerCase().includes(value)
              )
            })
          : data.slice(0, 6);
          ...
          {(focused || isOpen) &&
            ...
            initialResults.length > 0 && (
              <div
                ...
                {...getMenuProps()}
              >
                {initialResults.slice(0, 6).map((collection, index) => (
                  <Link
                    key={collection?.name}
                    href={`/collections/${collection?.contract_address}`}
                  >
                    <a
                      {...getItemProps({
                        key: collection?.name,
              ...
              {results?.slice(0, 6).map((collection, index) => (
                ...
                >
                  ...
                  >
                    <img
                      src={
                        collection?.imageUrl ?? 'https://via.placeholder.com/30'
                      }
                      alt={`${collection?.name}'s logo.`}
                      className="h-9 w-9 overflow-hidden rounded-full"
                    />
