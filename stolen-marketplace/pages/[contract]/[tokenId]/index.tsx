...
const Index: NextPage<Props> = ({ collectionId, tokenDetails }) => {
  ...
  const tokenName = `${token?.token?.name || `#${token?.token?.token_id}`}`
  ...
  const description = metadata.description(`${token.token?.collection?.description as string}`)
    ...
    <Layout navbar={{}}>
      ...
      <div className="col-span-full content-start space-y-4 px-2 md:col-span-4 lg:col-span-5 lg:col-start-2 lg:px-0 2xl:col-span-4 2xl:col-start-3 3xl:col-start-5 4xl:col-start-7">
        ...
        <div className="hidden space-y-4 md:block">
          <CollectionInfo collection={details.collection} details={details} />
...
export const getStaticProps: GetStaticProps<{
  ...
  const tokenDetails = (await assetRes.json()) as any
  ...
  return {
    props: { collectionId, tokenDetails: tokenDetails },
  }
}
